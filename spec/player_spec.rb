...
describe 'Player' do 
  ...
  describe '.hp' do
    it "returns a player's hit points (HP)" do
      expect(player_1.hp).to eq(60)
    end

    it 'returns the default HP value, provided no HP value was given to player' do
      expect(player_1.hp).to eq(Player::DEFAULT_HIT_POINTS)
    end
  end

  describe '.receive_damage' do
    it "reduces the player's HP by a random number (10) when attacked" do
